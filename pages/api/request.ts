...
export const Request = (url: string | null, repeatInterval?: number) => {
  const { data, error, isLoading } = useSWR(url ? url : null, fetcher, {
    ...
    // Some even say that this is the best way to communicate with API in Next.js, avoiding other hooks, but who am I to judge? :)
    refreshInterval: repeatInterval || 0,
    // I've decided to male news update news with a refreshInterval because I think adding useSWRSubscription wasn't necessary for this type of feature.
